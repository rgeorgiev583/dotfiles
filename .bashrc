...
# ALIAS {{{
  ...
  # GIT {{{
    ...
    alias gsshl='git stash list'
    alias gsshs='git stash show'
    alias gsshp='git stash pop'
    alias gssha='git stash apply'
    alias gsshd='git stash drop'
    alias gmv='git mv'
    alias grm='git rm'
    alias gpl='git pull'
  ...
  # YADM {{{
    if which yadm &>/dev/null; then
      ...
      alias yapp='yadm apply'
      alias yappp='yadm apply -p'
      alias ysh='yadm show'
      alias ygr='yadm grep'
      alias ybs='yadm bisect'
      alias yco='yadm checkout'
      alias ycop='yadm checkout -p'
      alias ycob='yadm checkout -b'
      alias yr='yadm reset'
      alias yrp='yadm reset -p'
      alias yrv='yadm revert'
      alias ybr='yadm branch'
      alias ybrr='yadm branch -r'
      alias yssh='yadm stash'
      alias ysshl='yadm stash list'
      alias ysshs='yadm stash show'
      alias ysshp='yadm stash pop'
      alias yssha='yadm stash apply'
      alias ysshd='yadm stash drop'
      alias ymv='yadm mv'
      alias yrm='yadm rm'
      alias ypl='yadm pull'
...
# FUNCTIONS {{{
  # BETTER GIT COMMANDS {{{
    ...
    {
        git submodule sync --recursive
        git pull "$@" && git submodule update --init --recursive --merge --remote
    }

    gfra()
    {
        git submodule sync --recursive
        git pull -r "$@" && git submodule foreach --recursive git pull -r "$@"
    }

    gfrsu()
    {
        git submodule sync --recursive
        git pull -r "$@" && git submodule update --init --recursive --rebase --remote
    }

    gsfra()
    {
        git submodule sync --recursive
        gsfr && git submodule foreach --recursive bash -c gsfr
    }
    ...
    ypla()
    {
        ...
        yadm pull "$@" && yadm submodule foreach --recursive yadm pull "$@"
    }

    yplsu()
    {
        yadm submodule sync --recursive
        yadm pull "$@" && yadm submodule update --init --recursive --merge --remote
    }

    yfra()
    {
        yadm submodule sync --recursive
        yadm pull -r "$@" && yadm submodule foreach --recursive yadm pull -r "$@"
    }

    yfrsu()
    {
        yadm submodule sync --recursive
        yadm pull -r "$@" && yadm submodule update --init --recursive --rebase --remote
    }

    ysfra()
    {
        yadm submodule sync --recursive
        gsfr && yadm submodule foreach --recursive bash -c gsfr
    }

    ypa()
    {
        yadm push && yadm submodule foreach --recursive yadm push
    }
